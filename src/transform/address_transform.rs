...
pub fn get_wasm_code_offset(loc: SourceLoc, code_section_offset: u64) -> WasmAddress {
    ...
    loc.get().wrapping_sub(code_section_offset as u32) as WasmAddress
}
...
impl<'a> Iterator for TransformRangeEndIter<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        while let Some((first, tail)) = self.indicies.split_first() {
            let range_index = *first;
            let range = &self.ranges[range_index];
            // changed from >= to prevent infinite loop
            // TOOD: review
...
mod tests {
    ...
    fn create_simple_func(wasm_offset: u32) -> CompiledFunctionData {
    ...
    fn create_simple_module(func: CompiledFunctionData) -> ModuleAddressMap {
    ...
    fn test_build_function_lookup_two_ranges() {
        ...
        input.instructions.push(CompiledInstructionData {
