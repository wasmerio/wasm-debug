...
pub struct FunctionFrameInfo<'a> {
    pub value_ranges: &'a ValueLabelsRangesInner,
    pub memory_offset: i64,
    // TODO: go back and repair this code (was originally a stackslot type which had a lot more info, current unused)
    pub stack_slot_offsets: &'a [Option<i32>],
}
    ...
    match loc {
        ...
        ValueLoc::Stack(ss) => {
            if let Some(frame_info) = frame_info {
                if let Some(ss_offset) = frame_info.stack_slot_offsets[ss.as_u32() as usize] {
